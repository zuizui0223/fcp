...
def main():
    ...
    raw.to_csv(out/"all_docx_rows.csv",index=False)
    result={"schema":"fcp_silene_cross_species_s1_preflight_v1","status":"complete","tables":manifest,"paragraphs":len(doc.paragraphs),"rule":"schema-only; no cross-species frequency contrast calculated"}
    (out/"result.json").write_text(json.dumps(result,indent=2)+"\n")
    print(json.dumps(result,indent=2))
    print(raw.head(150).to_string(index=False))
