...
def main():
    ...
    for var in ['bio5','bio14','srad_mean']:
        ...
        rows.append({'variable':var,'role':'posthoc_observer_paired_sensitivity','paired_observer_strata':int(len(od)),'species_with_paired_observer':int(len(vals)),'median_species_mean_paired_delta':float(np.median(vals)) if len(vals) else None,'mean_species_mean_paired_delta':float(np.mean(vals)) if len(vals) else None,'positive_species_fraction':float((vals>0).mean()) if len(vals) else None,'wilcoxon_two_sided_p':float(w.pvalue) if w else None,'sign_two_sided_p':float(sign.pvalue) if sign else None,**fitout})
        for sp,v in sd.items(): per_species.append({'variable':var,'species':sp,'mean_paired_observer_delta':float(v),'n_paired_observers':int((od.species==sp).sum())})
    result={'schema':'fcp_white_environment_observer_sensitivity_v1','status':'complete','role':'posthoc sensitivity after the primary WorldClim mechanism result was opened; cannot upgrade or redefine the prospective gate','design':'only species-observer strata containing both white and nonwhite outcomes; same frozen high-clip exclusion and near-clip adjustment','results':rows}
    pd.DataFrame(rows).to_csv(out/'observer_paired_sensitivity.csv',index=False)
    pd.DataFrame(per_species).to_csv(out/'observer_paired_species_deltas.csv',index=False)
