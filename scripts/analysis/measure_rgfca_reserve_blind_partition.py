#!/usr/bin/env python3
"""Measure a reserve partition with unchanged flower outputs plus a matched control.

Derived from the exact discovery worker at 9fae6ccdf684a46026f72ba12e98de2c5c54bf2a.
The only added measurement is the fixed background palette on the same masks.
...
def main() -> int:
    ...
    result_columns = [
        ...
        *[f"background_palette_count_{name}" for name in REFERENCE_RGB],
    ...
    for position, row in enumerate(worker.itertuples(index=False), start=1):
        ...
        background_counts = {name: 0 for name in REFERENCE_RGB}
        try:
            ...
            if roi_status != "automated_colour_state_admitted":
                ...
                background_counts = background_palette_counts(rgb, measured)
        ...
        result.update({f"background_palette_count_{name}": int(background_counts[name]) for name in REFERENCE_RGB})
        result.update(
            {
