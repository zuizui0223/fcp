...
from fcp_pipeline.third_cohort_prospective_execution_gate import (  # noqa: E402
    EXPECTED_ROWS,
    EXPECTED_SPECIES,
...
def units_from_vectors(table: pd.DataFrame) -> np.ndarray:
    ...
    if np.any(~np.isfinite(norms)) or np.any(norms <= EPS):
        raise RuntimeError("third-cohort H2 table contains invalid Delta vectors")
    return x / norms[:, None]
...
def measurement_support_from_receipt(measurement: dict) -> dict:
    if measurement.get("schema") != "third_cohort_prospective_measurement_result_v1":
        raise RuntimeError("unexpected third-cohort measurement receipt schema")
    if measurement.get("status") != "complete_third_cohort_location_blind_measurement_and_support_gate":
        raise RuntimeError("third-cohort measurement receipt is incomplete")
    if measurement.get("stage") != "SUPPORT_GATE_COMPLETE":
        raise RuntimeError("third-cohort measurement receipt did not reach SUPPORT_GATE_COMPLETE")
    if measurement.get("species") != EXPECTED_SPECIES or measurement.get("rows") != EXPECTED_ROWS:
        raise RuntimeError("third-cohort measurement receipt denominator mismatch")
    if measurement.get("H2_opened") is not False:
        raise RuntimeError("third-cohort measurement receipt says H2 was already opened")
    if measurement.get("replacement_rows", 0) != 0 or measurement.get("replacement_species", 0) != 0:
        raise RuntimeError("third-cohort measurement receipt contains forbidden replacement")
    if measurement.get("persisted_image_pixels") is not False:
        raise RuntimeError("third-cohort measurement receipt says image pixels persisted")

    n = measurement.get("measurement_evaluable_species")
    if type(n) is not int or not 0 <= n <= EXPECTED_SPECIES:
        raise RuntimeError("invalid third-cohort measurement-evaluable species count")
    expected = "PASS" if n >= MIN_MEASUREMENT_EVALUABLE_SPECIES else "NOT_EVALUABLE"
    if measurement.get("support_decision") != expected:
        raise RuntimeError("third-cohort measurement-support decision mismatch")
    support_stage = measurement.get("stage_receipts", {}).get("support")
    if not isinstance(support_stage, dict):
        raise RuntimeError("third-cohort measurement receipt lacks support-stage record")
    if support_stage.get("stage") != "SUPPORT_GATE_COMPLETE":
        raise RuntimeError("embedded third-cohort support-stage record is invalid")
    if support_stage.get("measurement_evaluable_species") != n:
        raise RuntimeError("embedded support count differs from third-cohort measurement receipt")
    if support_stage.get("support_decision") != expected:
        raise RuntimeError("embedded support decision differs from third-cohort measurement receipt")
    return {
        "pass": expected == "PASS",
        "decision": expected,
        "evaluable_species": n,
...
def write_result(out: Path, result: dict) -> dict:
    ...
    if reread != result:
        raise RuntimeError("third-cohort H2 result failed durable disk read-back")
    print(json.dumps(reread, indent=2, sort_keys=True, allow_nan=False))
    return reread
...
def main() -> int:
    ...
    if support["pass"] is not True:
        result = {
            "schema": "third_cohort_prospective_h2_result_v1",
            "analysis": "polymorphism_h2_third_cohort_prospective_white_axis",
            "date_jst": "2026-09-17",
    ...
    for label, threshold in THRESHOLDS.items():
        ...
        if len(table) != len(vectors):
            raise RuntimeError("third-cohort vector table and matrix have different denominators")
    ...
    result = {
        ...
        "structured_null": (
            "within third cohort and frozen coarse morph, permute normalized nine-colour rows across "
            "already selected H2 species while preserving species x coarse-morph row counts; refit "
            "unchanged label-free Hellinger two-means"
        ),
    ...
    reread = write_result(args.output_dir, result)
    validate_stage_transition(support["support_stage"], reread["stage_receipt"])
    return 0
