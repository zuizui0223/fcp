#!/usr/bin/env python3
"""Finalize the fixed five-block expanded environmental panel with Holm correction.

This script never recomputes a biological block. It combines exactly the five
predeclared raw block results and opens within-block decomposition only for a
parent block that survives the fixed five-member Holm family.
"""
...
def main() -> int:
    ...
    for name in EXPECTED:
        ...
        block_rows.append({
            ...
            "block_edge_occurrence_coverage_fraction": float(r["block_edge_occurrence_coverage_fraction"]),
    ...
    payload = {
        ...
        "status": "complete_fixed_five_block_environmental_panel_holm",
