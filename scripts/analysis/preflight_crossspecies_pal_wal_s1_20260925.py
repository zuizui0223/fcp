...
def main():
    ...
    result={
      "schema":"fcp_crossspecies_pal_wal_s1_preflight_v1",
      "status":"complete",
      "tables":manifest,
      "paragraphs":len(doc.paragraphs),
      "rule":"schema-only; no cross-species frequency statistic calculated"
    }
    (out/"result.json").write_text(json.dumps(result,indent=2)+"\n")
    print(json.dumps(result,indent=2))
    print(raw.head(160).to_string(index=False))
