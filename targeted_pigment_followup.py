#!/usr/bin/env python3
"""Second-pass OpenAlex search for unresolved flower-colour candidates."""
from __future__ import annotations
import argparse, csv, html, json, os, re, time, urllib.parse, urllib.request
from pathlib import Path

OPENALEX = "https://api.openalex.org/works"
TERMS = '"petal pigmentation" OR "corolla color" OR "corolla colour" OR anthocyanin OR albino OR "white flowered" OR "white-flowered"'
RELEVANT = re.compile(r"\b(?:flower|floral|petal|corolla|labellum|lip|anthocyanin|albino|white[- ]flowered)\b", re.I)
...
def request(url,timeout,retries):
    ...
    for n in range(retries):
        try:
            q=urllib.request.Request(url,headers={"User-Agent":"fcp-pigment-followup/1.0","Accept":"application/json"})
...
def main():
    ...
    ap.add_argument("--first-pass",default="data/targeted_followup_works.csv")
    ap.add_argument("--out",default="data/targeted_pigment_followup_works.csv")
    ap.add_argument("--qc-out",default="data/targeted_pigment_followup_qc.json")
    ...
    a=ap.parse_args()
    deferred=rows(a.deferred)
    prior={r["canonical_name"] for r in rows(a.first_pass)} if Path(a.first_pass).exists() else set()
    eligible=[]
    for r in deferred:
    ...
    api=os.environ.get(a.api_key_env,"").strip()
    out=[]; errors=[]; seen=set(); requests=0
    for bi in range(0,len(targets),a.batch_size):
        batch=targets[bi:bi+a.batch_size]
        genera=sorted({r["canonical_name"].split()[0] for r in batch})
        ...
        for item in results:
            ...
            for t in batch:
                ...
                score=14 + (10 if name.lower() in title.lower() else 0) + (4 if re.search(r"albino|white[- ]flowered|anthocyanin",text,re.I) else 0) - (10 if NEGATIVE.search(text) else 0)
    ...
    qc={"eligible_unresolved_targets":len(targets),"requests_used":requests,"failed_requests":len(errors),"errors":errors,"targets_with_new_evidence":len({r["canonical_name"] for r in out}),"retained_followup_works":len(out),"api_key_present":bool(api),"mode":"targeted_pigment_followup_v1"}
