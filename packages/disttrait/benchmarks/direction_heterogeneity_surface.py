#!/usr/bin/env python3
"""Direction-heterogeneity benchmark for continuous spatial traits.

The benchmark targets a model-misspecification/generalization distinction.

Each species has:
- a species-specific baseline trait level that changes geographically among species;
- a within-species local spatial effect;
- a sign that may reverse among species.

The disttrait matched-null statistic is based on pairwise trait dissimilarity and
is therefore agnostic to whether the trait increases or decreases along a local
spatial direction. The fixed-effect linear comparator assumes one common signed
within-species slope and is correctly specified only when directions are shared.
"""
...
def _fixed_effect_common_slope_test(
    ...
    This is equivalent to a species fixed-intercept linear model with one shared
    signed slope. The benchmark intentionally violates that common-slope
    assumption as the reversal fraction increases.
    """
    ...
    for z, y in groups:
        ...
        if z.shape != y.shape or z.ndim != 1 or len(z) < 3:
            raise ValueError("invalid fixed-effect group")
    ...
    statistic = beta / se
    p = float(student_t.sf(statistic, df))
    return beta, p
...
def run_world(
    *,
    ...
    for i, centre in enumerate(centres):
        ...
        # Strong between-species turnover creates the same pooled-confounding
        # pressure used in the earlier benchmarks.
        baseline = 0.8 * centre
        trait = (
            baseline
...
def run_benchmark(*, worlds_per_cell: int = WORLDS_PER_CELL) -> dict:
    ...
    return {
        ...
        "summary": {
            ...
            "effect_0_8_reversal0_5_common_slope_detection_range": detection_range(
                strong, "common_slope_detection", 0.5
            ),
        },
    }
