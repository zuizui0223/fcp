...
parse_args <- function(args) {
  out <- list(primary=NULL, paginated=NULL, opentree=NULL, dated_s1=NULL,
              dated_s2=NULL, dated_s3=NULL, outdir=NULL,
              replicates=100L, seed=20260803L)
  i <- 1L
  while (i <= length(args)) {
    key <- sub("^--", "", args[[i]])
...
fit_one <- function(data, tree, occurrence_dataset, metric, model_type,
                    replicate=NA_integer_, scenario=NA_character_) {
  ...
  if (nrow(d) < 20L || length(unique(d$among)) < 2L) stop("Not estimable")
  fit <- phyloglm(among ~ metric_z + effort_z, data=d, phy=tr,
...
metrics <- c("bio1_q95q05", "bio5_q95q05", "bio6_q95q05", "bio7_q95q05")
...
for (dataset_name in names(datasets)) {
  for (metric_index in seq_along(metrics)) {
    ...
    for (r in seq_len(args$replicates)) {
      tr <- normalise_tree(open_base, grafen=TRUE,
                           seed=args$seed + metric_index * 1000L + r)
    ...
    for (scenario in names(dated)) {
      ...
      rows[[k]] <- fit_one(d, tr, dataset_name, metric,
                           "dated_megaphylogeny", scenario=scenario)
      k <- k + 1L
    }
  }
}
...
write.csv(fits, file.path(args$outdir, "temperature_component_phylogenetic_fits.csv"), row.names=FALSE)

summary_rows <- list(); k <- 1L
for (dataset_name in unique(fits$occurrence_dataset)) {
  for (metric in unique(fits$metric)) {
    for (model_type in unique(fits$model_type)) {
      x <- fits[fits$occurrence_dataset == dataset_name &
                fits$metric == metric & fits$model_type == model_type, , drop=FALSE]
      if (!nrow(x)) next
...
write.csv(summary, file.path(args$outdir, "temperature_component_phylogenetic_summary.csv"), row.names=FALSE)
...
manifest <- list(
  ...
  seed=args$seed,
  model_formula="among ~ separately standardised temperature component + effort_z",
  ...
  interpretation_guard=paste(
    ...
    "They do not establish a causal thermal mechanism or morph-specific tolerance."
  )
)
writeLines(toJSON(manifest, pretty=TRUE, auto_unbox=TRUE, na="null"),
           file.path(args$outdir, "temperature_component_phylogenetic_manifest.json"))
print(summary)
