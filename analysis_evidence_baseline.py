#!/usr/bin/env python3
"""Supporting evidence diagnostics for the research macroecology pipeline."""
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--species',default='data/global_flower_colour_species_ranked.csv');ap.add_argument('--outdir',default='analysis_outputs/evidence');a=ap.parse_args();data=rows(a.species)
    ...
    def tier(r):
        ...
        return 'D_minimal'
    tiers=Counter(tier(r) for r in data);families=defaultdict(list)
    ...
    family_rows.sort(key=lambda r:(-r['candidate_species'],r['family']));write_csv(out/'family_evidence_summary.csv',list(family_rows[0]),family_rows)
    sensitivity=[]
    for tm in (0,1,2):
        for cm in (1,2,3):
            for sm in (14,18,24):
                n=sum(classify(r,tm,cm,sm) for r in data);sensitivity.append({'title_min':tm,'context_min':cm,'score_min':sm,'retained_species':n,'retained_fraction':round(n/len(data),6)})
    write_csv(out/'threshold_sensitivity.csv',list(sensitivity[0]),sensitivity)
    concentration=[];ordered=sorted(family_rows,key=lambda r:-r['candidate_species']);total=len(data);cum=0
    ...
    qc={'input_species':len(data),'unique_families':len(families),'evidence_tiers':dict(sorted(tiers.items())),'top_10_family_share':round(sum(r['candidate_species'] for r in ordered[:10])/len(data),6),'method':'supporting_evidence_diagnostics_v2'}
