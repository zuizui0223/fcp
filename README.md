...
- **Next independent measurement gate:** [target definition and source-integrity checks](docs/RGFCA_MEASUREMENT_TARGET_AND_VALIDATION_V2.md). The three-family follow-up distinguishes generic flower regions from focal petals; USDA public 100/30 split lists overlap by one filename (union 129), retained with exact-byte receipts and offline tests. No new image/model benchmark is admitted.
