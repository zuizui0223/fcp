...
def request(url,timeout,retries,base_delay):
    last=None
    for n in range(retries):
        try:
            ...
            if e.code==429 and retry_after:
                try: wait=max(float(retry_after),base_delay*(2**n))
                except ValueError: wait=base_delay*(2**n)
            else: wait=base_delay*(2**n)
        except Exception as e:
            last=e; wait=base_delay*(2**n)
