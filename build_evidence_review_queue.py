...
DIRECT_POLYMORPHISM_RE = re.compile(
    r"(?:flower|floral|petal|bract).{0,45}(?:colou?r).{0,45}(?:polymorph|morph|variation|dimorph)"
    r"|(?:polymorph|dimorph|morph).{0,45}(?:flower|floral|petal|bract).{0,45}(?:colou?r)",
    re.IGNORECASE,
)
...
FIELDS = [
    ...
    "natural_signal_count", "artificial_signal_count", "colour_change_signal_count",
...
def evidence_class(row: Dict[str, str]) -> tuple[str, int, int, int]:
    ...
    if direct and (natural >= 1 or to_int(row.get("n_context_matches")) >= 2):
...
def main() -> None:
    ...
    deferred.sort(key=lambda row: (
        ...
        -to_int(row.get("artificial_signal_count")),
        -to_int(row.get("colour_change_signal_count")),
        row.get("canonical_name", ""),
    ))
    ...
    qc = {
        ...
        "review_mode": "natural_population_polymorphism_v2",
    }
