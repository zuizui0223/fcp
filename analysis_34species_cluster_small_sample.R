...
for (metric in metrics) {
  ...
  est_col <- pick(c("^beta$","estimate","coef"))
  se_col <- pick(c("^SE$","std","se"))
  df_col <- pick(c("Satt.*df","d.f.*Satt","df_Satt","d.f"))
  p_col <- pick(c("Satt.*p","p.*Satt"))
  if (any(is.na(c(est_col,se_col,df_col,p_col)))) {
    stop(paste("Unsupported clubSandwich output columns:", paste(cn, collapse=", ")))
  }
  est <- as.numeric(target[[est_col]])
  se <- as.numeric(target[[se_col]])
  df_s <- as.numeric(target[[df_col]])
  p_s <- as.numeric(target[[p_col]])
  ...
  rows[[metric]] <- data.frame(
    ...
    cr2_ci_low=exp(est-crit*se),
    cr2_ci_high=exp(est+crit*se),
...
manifest <- list(
  ...
  interpretation_guard="CR2/Satterthwaite is a small-cluster sensitivity analysis. It complements, rather than replaces, permutation, leave-one-family-out, and phylogenetic analyses."
)
json <- paste0(
