...
We combined source-traceable, rule-derived literature classifications with GBIF occurrences and WorldClim 2.1 data. Binomial models used family-clustered uncertainty, label permutations and family deletion. We repeated the focal model with a paginated, quality-filtered GBIF sample and fitted phylogenetic logistic models using both an Open Tree topology and a time-scaled vascular-plant megaphylogeny.
