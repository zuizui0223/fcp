...
Reserve **flower-colour polymorphism** for its stricter population-level meaning: coexistence of at least two discrete flower-colour variants within the same population.

This terminology is especially important when linking `fcp` to `chun`, because lineage-level repeated colour evolution is not itself a polymorphism unless local coexistence is documented.
